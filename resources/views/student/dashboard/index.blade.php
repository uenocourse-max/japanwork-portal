...
@section('content')
    @php
        $statusLabels = [
            ...
            'reviewed' => ['label' => 'Direview', 'color' => 'bg-blue-100 text-blue-800'],
            'accepted' => ['label' => 'Diterima', 'color' => 'bg-indigo-100 text-indigo-800'],
            'interview_scheduled' => ['label' => 'Interview', 'color' => 'bg-purple-100 text-purple-800'],
            'company_accepted' => ['label' => 'Diterima Perusahaan', 'color' => 'bg-green-100 text-green-800'],
            'not_passed' => ['label' => 'Tidak Lolos', 'color' => 'bg-red-100 text-red-800'],
        ];

        $matchingLabels = [
            'not_matched' => ['label' => 'Belum Match', 'color' => 'bg-gray-100 text-gray-600'],
            'in_progress' => ['label' => 'Dalam Proses', 'color' => 'bg-blue-100 text-blue-800'],
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            @php
                $matching = $matchingLabels[$student->matching_status] ?? ['label' => 'Belum Match', 'color' => 'bg-gray-100 text-gray-600'];
            @endphp
            <div class="text-center py-4">
                <span class="inline-block px-4 py-2 rounded-full text-sm font-semibold {{ $matching['color'] }}">
                    {{ $matching['label'] }}
                </span>
                @if ($student->matching_status === 'matched' && $student->matched_company_name)
                    <p class="text-sm text-gray-600 mt-2">Perusahaan: <strong>{{ $student->matched_company_name }}</strong></p>
                @endif
            </div>
            @if ($student->matching_status !== 'matched')
            ...
            <div class="divide-y">
                @foreach ($recentApplications as $app)
                    <div class="py-3 flex justify-between items-center">
                        ...
                        @php $status = $statusLabels[$app->status] ?? ['label' => $app->status, 'color' => 'bg-gray-100 text-gray-600']; @endphp
